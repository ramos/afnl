...
A real double or simple precision (same type as the input).
...
Real Single or Double precision, the same as the input values.
...
\begin{displaymath}
  Y = \sum_i a_i f_i(X)
\end{displaymath}
the values $f_i(X)$ are given y this function as \texttt{Func(X,
  I)}. If the functions are not specified (i.e. you dont put this
argument), a fit to a polynomial is made.
